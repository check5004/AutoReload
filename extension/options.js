...
const params=new URLSearchParams(location.search);let origin=C.profileScope(params.get('origin')),tabId=Number(params.get('tab'))||undefined,profile=C.defaults(),changed=false;
const tabScope=url=>C.tabScope(url,chrome.runtime.getURL(''));
...
function paint(){
  ...
  $('clear-ready').hidden=!profile.readyTarget;$('enabled-status').textContent=(origin===C.PRACTICE?'練習ページ':'このドメイン')+(profile.enabled?'はONです':'はOFFです');renderSteps();
}
...
async function targetTab(){if(tabId){try{const tab=await chrome.tabs.get(tabId);if(await C.resolveTabScope(tab,chrome.runtime)===origin)return tabId;}catch{}}
  const candidates=origin===C.PRACTICE?await C.practiceTabs(chrome.runtime,chrome.tabs):(await chrome.tabs.query({url:C.pattern(origin)})).filter(t=>tabScope(t.url)===origin);if(candidates.length!==1)throw new Error('操作したいページで拡張機能を開き「設定・記録した操作」からこの画面を開き直してください');tabId=candidates[0].id;return tabId;
}
...
(async()=>{try{const list=await send('LIST');if(origin&&!list.some(x=>x.origin===origin))list.push({origin,profile:C.defaults()});$('domains').innerHTML=list.map(x=>`<option value="${esc(x.origin)}">${esc(C.scopeLabel(x.origin))}</option>`).join('');origin=origin||list[0]?.origin;$('no-domain').hidden=!!origin;$('profile-form').hidden=!origin;if(origin){$('domains').value=origin;await load();}await showUpdates();}catch(e){error(e);}})();
