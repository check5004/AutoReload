...
async function refresh(){if(!origin)return;const data=await send('GET');enabled=data.profile.enabled;$('toggle').setAttribute('aria-checked',String(enabled));$('status').textContent=!enabled?'OFF · このページでONにすると使えます':data.run?.message||'待機中 · 必要なときに監視を開始';for(const id of ['watch','execute','stop','pick','ready'])$(id).disabled=!enabled;}
$('version').textContent='v'+chrome.runtime.getManifest().version;
$('toggle').onclick=async()=>{try{$('error').textContent='';if(!enabled&&origin!==AR.PRACTICE){const granted=await chrome.permissions.request({origins:[AR.pattern(origin)]});if(!granted)return;}await send('ENABLE',{enabled:!enabled});await refresh();}catch(e){showError(e);}};
...
(async()=>{try{[tab]=await chrome.tabs.query({active:true,currentWindow:true});origin=tab?await AR.resolveTabScope(tab,chrome.runtime):null;if(!origin){$('domain').textContent='このページは対象外です';$('status').textContent='購入ページや内蔵の練習ページを開いてください';for(const id of ['watch','execute','stop','pick','ready'])$(id).disabled=true;}else{$('domain').textContent=AR.scopeLabel(origin);$('toggle').disabled=false;await refresh();}await update();}catch(e){showError(e);}})();
